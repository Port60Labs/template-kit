...
function islandSkeleton(name, ctx = {}, fx = STUDIO_FX) {
  ...
  switch (name) {
    case 'donation_widget':
      return `<section class="donate-card action-card donation-widget" data-p60-preview-island="donation_widget" aria-label="Donation form preview. Controls are disabled.">
        <span class="p60-preview-badge">Preview only</span>
        <header class="donate-head"><h2>Make a donation</h2></header>
        <div class="freq-tabs" role="group" aria-label="Donation frequency"><button class="freq-tab selected" type="button" aria-pressed="true" disabled>One-time</button><button class="freq-tab" type="button" aria-pressed="false" disabled>Monthly</button></div>
        <div class="amount-grid" role="group" aria-label="Donation amount"><button class="amount-option" type="button" aria-pressed="false" disabled>£10</button><button class="amount-option selected" type="button" aria-pressed="true" disabled>£25</button><button class="amount-option" type="button" aria-pressed="false" disabled>£50</button></div>
        <label class="field custom-amount"><span>Or enter your own amount</span><div class="amount-input"><span class="currency" aria-hidden="true">£</span><input inputmode="decimal" placeholder="Other amount" disabled></div></label>
        <div class="donor-fields">
          <label class="field"><span>Name on card</span><input placeholder="Your name" disabled></label>
          <label class="field"><span>Email</span><input type="email" placeholder="you@example.com" disabled></label>
          <label class="marketing-optin"><input type="checkbox" disabled><span>Email me about this charity's work and appeals. Unsubscribe any time, in one click.</span></label>
        </div>
        <div class="donation-giftaid-controls"><div class="donation-giftaid-summary-actions">
          <button class="donation-giftaid-trigger" type="button" aria-label="Add Gift Aid" disabled><span class="donation-giftaid-action">Add</span>${GIFT_AID_LOGO}</button>
        </div></div>
    ...
    case 'primary_action_widget':
      ...
      if (fixtures.focus === 'none') {
        return `<div class="p60-preview-empty" data-p60-preview-island="primary_action_widget">${previewNote(name)}<p>No widget here: this charity leads with buttons only (site.actions.widget is 'none').</p></div>`;
      }
...
function eventsListingSkeleton(fx = STUDIO_FX) {
  ...
  return `${surfaceDivider('events')}<section class="section"><div class="container">
    <h1>${escapeHtml(fx.site?.content.events.label ?? 'Events')}</h1>
...
function articlesListingSkeleton(fx = STUDIO_FX) {
  ...
  return `${surfaceDivider('articles')}<section class="section"><div class="container">
    <h1>${escapeHtml(fx.site?.content.articles.label ?? 'Articles')}</h1>
...
function servicesListingSkeleton(fx = STUDIO_FX) {
  ...
  return `${surfaceDivider('services')}<section class="section"><div class="container">
    <h1>${escapeHtml(fx.site?.content.services.label ?? 'Services')}</h1>
...
function courseListingSkeleton(fx = STUDIO_FX) {
  ...
  return `${surfaceDivider('courses')}<section class="section"><div class="container">
    <h1>${escapeHtml(fx.site?.content.courses.label ?? 'Courses')}</h1>
...
const SURFACES = {
  ...
  services: { template: 'services', builtin: servicesListingSkeleton },
  ...
  campaigns: { template: 'campaigns', builtin: campaignsListingSkeleton },
  campaign: { template: null, builtin: campaignDetailSkeleton },
  courses: { template: 'courses', builtin: courseListingSkeleton },
...
export async function renderStudioPreview(files, options = {}) {
  ...
  fx = {
    ...
    pages: {
      ...fx.pages,
      events: { events: c.events.items },
      articles: { articles: c.articles.items },
      course: { ...fx.pages?.course, course: c.courses.items.length ? { ...fx.pages?.course?.course, ...c.courses.items[0] } : fx.pages?.course?.course }
    },
    sections: {
      ...fx.sections,
      events: { events: c.events.items },
      whatsOn: { infoEvents: c.events.items.filter(event => event.registrationMode === 'INFO') },
      articles: { latestArticles: c.articles.items },
      campaigns: { campaigns: c.campaigns.items, campaignsLabel: c.campaigns.label },
      appealGrid: { causes: c.causes.items }, emergency: { causes: c.causes.items },
      programmes: { services: c.services.items }, resources: { resources: c.documents.items }
    }
  };
  ...
  if (surface !== 'home' && SURFACES[surface]) {
    ...
    if (templateSource != null && fixture != null
      ...
      const context = { site, ...(['article', 'course'].includes(def.template) ? fixture : {}) };
    ...
    const renderComposition = async (composition) => {
      ...
      for (const entryInPage of composition) {
        const { type, content } = entryInPage;
    ...
    for (const page of options.surface ? [] : (manifest?.supports?.pageTemplates ?? [])) {
      ...
      const context = { site: { ...site, page: { key: page, path: '/' + page, sections: [] } }, ...(['article', 'course'].includes(page) ? fixture : {}) };
      const rendered = await liquid.parseAndRender(source, context);
  ...
  if (manifest?.supports?.layout && files['layout.liquid'] != null) {
    const rendered = await liquid.parseAndRender(files['layout.liquid'], { site });
